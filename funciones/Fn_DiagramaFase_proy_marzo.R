...
#	png(paste(Salida3,"/DF_",name,".png",sep=""),width=450,height=400)        
  ...
  mtext(side=2.9,line=-8,"Sobre-explotacin y ",cex=1.2,adj = 0.8)
	...
	arrows(x0=B95[1],y0=lastF,x1=B95[2],y1=lastF,length=0.05,angle=90,col="red",lwd=1,code=3)
	arrows(x0=lastB,y0=F95[1],x1=lastB,y1=F95[2],length=0.05,angle=90,col="red",lwd=1,code=3)

	points(c(tail(Bval,1),tail(Bval[1])),c(tail(Fval,1),tail(Fval[1])),pch=16,cex=1,col=c("red","orange"))
